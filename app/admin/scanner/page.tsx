    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-50">
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <h1 className="text-sm font-semibold text-slate-900">QRスキャン受付</h1>
        </div>
      </header>
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-3">
          <p className="text-xs text-slate-500 mb-2">受付モード</p>
          ...
          <div className="relative">
            ...
            {!cameraActive && scanState !== 'fetching' && scanState !== 'result' && scanState !== 'checking_in' && scanState !== 'awaiting_verification' && scanState !== 'success' && (
              <div className="flex flex-col items-center justify-center py-16 px-4">
                ...
                <Button
                  onClick={startCamera}
                  className="bg-indigo-600 hover:bg-indigo-700 text-white gap-2 h-11 px-6 rounded-xl"
                >
          ...
          {cameraActive && (
            <div className="flex items-center justify-between px-4 py-3 bg-indigo-50 border-t border-indigo-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-indigo-700 font-medium">スキャン中...</span>
              </div>
              <Button
                variant="ghost"
                size="sm"
        ...
        {scanState === 'fetching' && (
          <div className="bg-white rounded-xl border border-slate-200 p-8 flex flex-col items-center gap-3">
            <Loader2 className="h-8 w-8 animate-spin text-indigo-500" />
            <p className="text-sm text-slate-500">参加者情報を取得中...</p>
          </div>
        )}
        ...
        {scanState === 'checking_in' && (
          <div className="bg-white rounded-xl border border-slate-200 p-8 flex flex-col items-center gap-3">
            <Loader2 className="h-8 w-8 animate-spin text-indigo-500" />
            <p className="text-sm text-slate-500">チェックイン処理中...</p>
          </div>
        )}
        ...
        {scanState === 'awaiting_verification' && responseData && (
          <div className="bg-white rounded-xl border-2 border-amber-300 p-6 space-y-4">
            <div className="flex items-center gap-2 text-amber-600">
              ...
              <span className="text-sm font-semibold">名前を確認してください</span>
            </div>
          ...
          <div className={`rounded-xl border-2 p-6 space-y-4 ${
            ...
            <div className="text-center space-y-3">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto ${
                alreadyCheckedIn ? 'bg-blue-100' : 'bg-emerald-100'
              }`}>
              ...
              {alreadyCheckedIn ? (
                <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-700 rounded-full px-4 py-2 text-sm font-medium">
                  ...
                  既に受付済みです
                  {checkedInAt && (
                    <span className="text-blue-500">({formatDateTime(checkedInAt)})</span>
                  )}
                </div>
              ) : (
            ...
            <Button
              onClick={resetScanner}
              className="w-full h-12 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold gap-2"
            >
        ...
        {scanHistory.length > 0 && (
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
              ...
              <span className="text-xs font-medium text-slate-500">スキャン履歴</span>
              <Badge variant="secondary" className="ml-auto bg-slate-100 text-slate-500 text-[10px]">
