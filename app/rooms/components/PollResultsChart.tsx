...
export default function PollResultsChart({
  ...
  return (
    <div className={`space-y-${large ? '4' : '3'}`}>
      {options.map((option, i) => {
        ...
        return (
          <div key={i}>
            ...
            <div className={`w-full ${large ? 'h-8' : 'h-6'} ${large ? 'bg-gray-100' : 'bg-slate-100'} rounded-lg overflow-hidden`}>
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${width}%` }}
      ...
      <p className={`${large ? 'text-sm' : 'text-xs'} text-gray-400 text-center`}>
        投票数: {totalVotes}
      </p>
    </div>
  );
