...
const HOST_FAQ_SECTIONS: Array<{
  ...
  },
  {
    ...
    body:
      '出席フォームを紐づけると、Q&Aや投票と同じ参加者画面のタブに「出席」が表示され、そこから出席登録できます。位置情報チェックや送信クールダウンなど、既存の出席フォーム設定はそのまま適用されます。',
    tips: ['紐づけ可能なフォームがない場合は、先にフォーム管理で作成してください。', '紐づけはいつでも解除できます。', '講師は参加者に「出席タブを開いてください」とアナウンスするだけで出席を取れます。'],
  },
  {
    id: 'report',
    title: 'レポート機能',
...
export default function HostPage() {
  ...
  const [tab, setTab] = useState<HostTab>(
    isHostTab(requestedTab) ? requestedTab : firstRunRequested ? 'polls' : 'questions'
  );
  ...
  useEffect(() => {
    if (isHostTab(requestedTab)) {
      setTab(requestedTab);
    } else if (firstRunRequested) {
      setTab('polls');
    }
  }, [requestedTab, firstRunRequested]);
