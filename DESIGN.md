...
- 管理画面・ホスト管理画面のページヘッダーは freee会計アプリの一覧画面に倣い、淡い青の横帯（`#edf4ff`）+ 下線（`#dce8ff`）+ 左アイコン + 画面名の構成にする。大きなピル型ラベルやグラデーションのヒーロー見出しは使わない。
- ヘッダー内の主要操作は右寄せで、プライマリ操作は `#2864f0`、角丸 4px-8px、補助操作は白背景 + `#e1dcdc` 境界線を使う。
...
- 管理画面の検索欄は左に検索アイコン、白背景、`#cccccc` の 1px 境界線、角丸 4px-8pxを使う。検索はカード一覧の直前に置き、該当なしの場合は白背景の空状態を表示する。
...
- 管理画面・ホスト管理画面の説明カードはデフォルト閉じた折りたたみカードにする。見出し行は `#ebf3ff` 背景、`#aac8ff` 境界線、本文色 `#23418c` を基本にし、開閉アイコンは `ChevronDown` / `ChevronUp` を使う。
